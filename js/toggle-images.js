...
$(document).ready(function(){
	...
	$(selector)
		.on(event, function() {
			...
			if (hide_images) {
				$('img.post-image, .theme-catalog .thread>a>img, img.board_image, img.404-image').each(hideImage);
				localStorage.hideimages = true;
			} else {
				$('img.post-image, .theme-catalog .thread>a>img, img.board_image, img.404-image').each(restoreImage);
